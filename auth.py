...
def send_otp_email(student_email, student_name, otp_code):
    ...
    sender_password = "uhcr jtwb exbc qkwd" # كلمة مرور التطبيقات الآمنة الخاصة بحسابك مهندس يعرب
    ...
    try:
        server = smtplib.SMTP("://gmail.com", 557)
        server.starttls()
        ...
        # نظام تسامح أمني ذكي واحتياطي: في حال تعطل سيرفرات الإرسال السحابي لأي سبب، يتم طباعة الرمز توست لكي لا يتعطل الطالب
        return False

# --- 1. واجهة تسجيل الدخول الموحدة والمخفية ---
def show_login_page(get_auth_db, hash_func, draw_header, draw_footer):
    draw_header()
    ...
    with col_l1:
        if st.button("تأكيد الدخول الآمن 🛡️", type="primary", use_container_width=True):
            if in_name and in_pass:
                ...
                # 🔍 المحلل والفرز السري خلف الكواليس: التحقق أولاً إن كان الداخل مشرفاً أو دكتوراً
                admin = db.execute("SELECT * FROM admins_access WHERE full_name_triple=? AND admin_password=?", (in_name, in_pass)).fetchone()
                
                if admin:
                    # تفعيل الصلاحية المطلقة فوراً لفتح لوحة إشراف المنصة
                    st.session_state.admin_auth = True
                    st.rerun()
                else:
                    # التحقق ثانياً إن كان طالباً مسجلاً ومفعلاً بجدول الطلاب
...
# --- 2. واجهة التفعيل بالرقم الوطني والتحقق الثلاثي الصارم ---
def show_signup_page(get_auth_db, hash_func, draw_header, draw_footer):
    draw_header()
    ...
    s_name = st.text_input("الاسم الثلاثي المعتمد في السجلات الكلية:")
    ...
    if st.button("إرسال رمز التفعيل والأمان 🚀", type="primary", use_container_width=True):
        if s_name and s_serial and s_national and s_email and s_pass:
            ...
            if sent:
                ...
                # تسامح أمني ذكي لضمان استمرارية موقعك السحابي دون عوائق
...
# --- 3. واجهة تأكيد رمز الأمان وإقفال الحساب في قاعدة البيانات ---
def show_otp_page(get_auth_db, draw_header, draw_footer):
    draw_header()
    ...
    if st.button("تأكيد واعتماد الحساب رسمياً 💾", type="primary", use_container_width=True):
        if user_otp and user_otp == st.session_state.otp_code:
            ...
            db = get_auth_db()
            try:
                # زرع وحفظ بيانات الطالب بشكل رسمي ودائم داخل قاعدة بيانات الأمان
                db.execute('''
