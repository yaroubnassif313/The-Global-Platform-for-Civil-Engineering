...
st.set_page_config(
    ...
    layout="centered",
    initial_sidebar_state="collapsed" # إغلاق وإلغاء القوائم الجانبية الحمقاء نهائياً
)
...
st.markdown("""
    ...
    * { translate: no !important; }
    ...
    /* تصميم البطاقات التفاعلية ذات الحواف الدائرية الكحلي الملكي */
    .stButton>button {
        width: 100%;
        border-radius: 15px;
        height: 4.5em;
    ...
    /* أيقونة الدعم الفني الدائمة والأنيقة في زاوية الميدل بار اليمنى دون إزعاج */
    .floating-support {
        position: fixed;
        bottom: 20px;
        right: 20px;
...
# --- 6. زر الدعم الفني الزاويّ الصغير والدائم عبر كافة الصفحات ---
with st.container():
    if st.button("📞 الدعم التقني", key="floating_support_btn", type="secondary"):
...
# فحص الأمان العالي: إذا تأكد نجاح دخول الدكتور كأدمن، يتم فتحه اللوحة الوردية فوراً وبسرية كاملة
if st.session_state.admin_auth:
    ...
    # تدفق واجهات الطلاب والزوار العادية
    if st.session_state.step == 'welcome':
        draw_global_header()
        
        # عرض الهيكل القيادي للكلية بأناقة في واجهة الاستقبال
        ...
        if st.session_state.lang == 'العربية':
            ...
            with col_b1:
                if st.button("🚀 Civil Engineering Explorer Gateway"):
                    ...
                    st.rerun()
            with col_b2:
                if st.button("🔑 Platform Secure Unified Login"):
                    st.session_state.step = 'login'
                    st.rerun()
        draw_global_footer()
