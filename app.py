...
st.set_page_config(
    ...
    layout="centered", 
    initial_sidebar_state="expanded" # تم جعل السايد بار مفتوحًا تلقائيًا عند البداية لتسهيل الرؤية والتجربة
)
...
# --- 5. استدعاء الملفات الفرعية الموازية بذكاء الحماية السحابية ---
try: import sidebar
...
# --- 6. تشغيل السايد بار الحاسم (تم إخراجه هنا ليعمل فوراً وبقوة بالتوازي دون قيود) ---
if sidebar:
    sidebar.show_sidebar()

# --- 7. المحرك والموجه الرئيسي لإدارة تنقل واجهات الطلاب والمسؤول ---
if st.session_state.is_admin_login:
    # تفعيل لوحة تحكم المشرفين والدكاترة (مستقبلية)
    if admin_panel:
    ...
    if st.session_state.step == 'welcome':
        if auth:
            ...
            # واجهة ترحيبية احتياطية ذكية تعمل تلقائيًا لحين رفع ملف auth.py
            draw_global_header()
        ...
        if auth:
            ...
            draw_global_header()
            st.subheader("📝 واجهة التفعيل بالرقم الوطني قيد الرفع الموازي...")
            if st.button("⬅️ عودة للخلف"):
                st.session_state.step = 'login'
                st.rerun()
            draw_global_footer()
