...
def show_sidebar(get_db_func=None):
    ...
    تمت إضافة (get_db_func=None) ليتوافق 100% وبثبات مع استدعاء كود app.py الثابت.
    """
    # فحص المرحلة الحالية: يعرض السايد بار فقط في صفحة welcome ولم يسجل دخول كمشرف بعد
    if st.session_state.step == 'welcome' and not st.session_state.is_admin_login:
        with st.sidebar:
            st.title("🏛️ بوابة التحكم")
            st.caption("المنصة الكبرى لجامعة اللاذقية")
            st.divider()
            
            # 🔐 زر تسجيل الدخول كمشرف (ينقل السيرفر بالتوازي لملف الإدارة)
